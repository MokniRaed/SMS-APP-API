...
export const uploadCategories = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error uploading categories", error: error.message });
  }
};
...
export const exportCategories = async (req, res) => {
  try {
    ...
    const categories = await Category.find({}).select(
      "cat_id name label type parentId -_id"
    ...
    // Convert categories to an array of objects for Excel export
    const data = categories.map((category) => ({
      categoryId: category.cat_id,
      name: category.name || "N/A",
      label: category.label || "N/A",
      type: category.type || "N/A",
      parentId: category.parentId || "N/A", // Handle potential nulls
      createdAt: category.createdAt
        ? category.createdAt.toISOString().split("T")[0]
        : "N/A", // Safely format createdAt
      updatedAt: category.updatedAt
        ? category.updatedAt.toISOString().split("T")[0]
        : "N/A", // Safely format updatedAt
    }));
    ...
    res
      .status(500)
      .json({ message: "Error exporting categories", error: error.message });
  }
};
